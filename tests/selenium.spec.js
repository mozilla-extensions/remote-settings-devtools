...
describe("End to end browser tests", () => {
  test("Load extension, change environment to prod, sync and clear all", async() => {
    ...
    expect((await driver.findElements(By.css('#status .unsync'))).length).toBeGreaterThan(1);
    expect((await driver.findElements(By.css('#status .up-to-date'))).length).toBe(0);
  });
  ...
  test("Clear and re-download a collection", async() => {
    ...
    await waitForLoad();
    let firstTimestamp = await driver.findElement(By.css('#status .human-local-timestamp'));
    expect(await firstTimestamp.getAttribute('class')).toContain('up-to-date');

    // force sync the first collection and verify it worked
    await clickByCss('#status .clear-data');
    await waitForLoad();
    firstTimestamp = await driver.findElement(By.css('#status .human-local-timestamp'));
